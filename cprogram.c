...
bool getAccelData() {
    int t,z;
    if (fscanf(fp, "%d %d", &t, &z) != EOF) {
//        printf("%d %d\n", t, z);
        data_point[0] = t; //time
        data_point[1] = z; //z accel
        return(true);
    }
    return(false);
...
void updateCounter(int t, float z) {
    hit_counter += 1;
    printf("Num_hits: %d \t t: %d \t z: %f \n", hit_counter, t, z);
}

/* helper functions (normal c doesn't have it)*/
...
/* from this part on, copied the corresponding functions
from bagCounter.ino*/
